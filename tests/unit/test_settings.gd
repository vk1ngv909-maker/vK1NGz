...
func _init() -> void:
	...
	_check(clean["numeral_style"] == "western", "western numerals are the default")
	var arabic_digits: Dictionary = settings.sanitize({"language": "ar", "numeral_style": "arabic_indic"})
	settings.load_values(arabic_digits)
	_check(settings.numeral_style == "arabic_indic", "Arabic-Indic numeral preference loads")
	_check(settings.format_number(12345) == "١٢٣٤٥", "all digits are converted for display")
	settings.set_value("numeral_style", "western", false)
	_check(settings.format_number(12345) == "12345", "Arabic locale defaults can retain western display digits")
	var missing_marker: String = settings.t("test.intentionally_missing_translation")
	_check(not OS.is_debug_build() or missing_marker == "!!test.intentionally_missing_translation!!", "missing translations are visibly marked in debug builds")
