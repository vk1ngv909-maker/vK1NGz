...
func _init() -> void:
	...
	ck("recovered data usable", rec.has("stage"))
	...
	var dd = SM.load()
	ck("falls back to defaults", SM.last_load_source == "default" and dd["stage"] == 1)
	...
	var m = SM.migrate(v1)
	ck("migrated to v2", m["schema_version"] == 2, str(m.get("schema_version")))
	ck("gold became BigNumber dict", typeof(m["gold"]) == TYPE_DICTIONARY, str(m["gold"]))

	# 6. offline: cap, rollback, and NO duplicate grant
	wipe()
	var od = SM.default_data()
	od["last_seen_utc"] = 1000
	od["offline_claimed_utc"] = 0
	SM.save(od)
	SM.load()
